...
public class MouseHook : IDisposable
{
    ...
    public void Dispose()
    {
        ...
        {
            if (!NativeMethods.UnhookWindowsHookEx(hookPtr)) throw new Win32Exception("Unable to clear MouseHook");
            hookPtr = IntPtr.Zero;
        }
    }
    ...
    public void StartHook()
    {
        hook = (nCode, wParam, lParam) =>
        {
            if (nCode >= 0)
            {
                ...
                if (args != null) MouseEvent?.Invoke(args);
            }
        ...
        if (hookPtr == IntPtr.Zero) throw new Win32Exception("Unable to set MouseHook");
    }
}

...
public static class DPadHelper
{
    ...
    /// </summary>
    public static DPadDirection[] Values { get; } = ((DPadDirection[])Enum.GetValues(typeof(DPadDirection)))
        .Where(d => d != DPadDirection.None).ToArray();
    ...
    {
        var value = DPadDirection.None;
        if (up)
            value |= DPadDirection.Up;
        else if (down) value |= DPadDirection.Down;

        if (right)
            value |= DPadDirection.Right;
        else if (left) value |= DPadDirection.Left;

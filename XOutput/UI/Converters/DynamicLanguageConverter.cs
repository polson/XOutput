...
public class DynamicLanguageConverter : IMultiValueConverter
{
    ...
    public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        if (values[1] is Enum)
        {
            key = values[1].GetType().Name + "." + values[1];
        ...
        else if (values[1] is sbyte || values[1] is byte || values[1] is char || values[1] is short ||
                 values[1] is ushort || values[1] is int || values[1] is uint || values[1] is long ||
                 values[1] is ulong || values[1] is decimal)
        {
    ...
    {
        if (translations == null || key == null || !translations.ContainsKey(key)) return null;

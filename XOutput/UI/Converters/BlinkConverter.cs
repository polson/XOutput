...
public class BlinkConverter : IMultiValueConverter
{
    ...
    public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        if (back)
        {
            if (currentType == activeType) return highlight == true ? Visibility.Visible : Visibility.Hidden;
            return Visibility.Hidden;
        }

        if (currentType == activeType) return highlight == true ? Visibility.Hidden : Visibility.Visible;
        return Visibility.Hidden;
    }

...
public class ColorConverter : IMultiValueConverter
{
    ...
    protected Dictionary<XInputTypes, Brush> backgroundColors = new();
    protected Dictionary<XInputTypes, Brush> foregroundColors = new();
    protected Dictionary<XInputTypes, Brush> labelColors = new();
    ...
    {
        var activeType = values[0] as XInputTypes?;
        var highlight = values[1] as bool?;
        var parameters = (parameter as string).Split('|');
        var back = parameters.Length > 1 && parameters[1] == "back";
        var label = parameters.Length > 1 && parameters[1] == "label";
        if (parameters[0] == "DPAD")
        {
            if (back)
            {
                if (highlight == true && XInputHelper.Instance.IsDPad(activeType.Value)) return HighlightBackBrush;
        ...
        else
        {
            ...
            if (back)
            {
                ...
                if (backgroundColors.ContainsKey(currentType)) return backgroundColors[currentType];
            }
            else if (label)
            {
                if (highlight == true && currentType == activeType)
                    return HighlightLabelBrush;
                if (labelColors.ContainsKey(currentType)) return labelColors[currentType];
            }
            else
            {
                if (highlight == true && currentType == activeType)
                    return HighlightBrush;
                if (foregroundColors.ContainsKey(currentType)) return foregroundColors[currentType];
            }
        }

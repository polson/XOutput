...
public class InputSettingsViewModel : ViewModelBase<InputSettingsModel>, IDisposable
{
    ...
    public InputSettingsViewModel(InputSettingsModel model, IInputDevice device) : base(model)
    {
        this.device = device;
    ...
    {
        foreach (var axisView in Model.InputAxisViews) axisView.UpdateValues(device);
        foreach (var buttonView in Model.InputButtonViews) buttonView.UpdateValues(device);
        foreach (var dPadView in Model.InputDPadViews) dPadView.UpdateValues(device);
    }

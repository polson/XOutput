...
public class ControllerSettingsViewModel : ViewModelBase<ControllerSettingsModel>, IDisposable
{
    ...
    public ControllerSettingsViewModel(ControllerSettingsModel model, GameController controller) :
        base(model)
    {
        ...
        foreach (var device in devices)
        {
            ...
            if (!string.IsNullOrEmpty(controller.Mapper.ForceFeedbackDevice) &&
                controller.Mapper.ForceFeedbackDevice == device.DisplayName) Model.ForceFeedback = item;
        }
    }
    ...
    public void ConfigureAll()
    {
        ...
        new AutoConfigureWindow(
            new AutoConfigureViewModel(new AutoConfigureModel(), InputDevices.Instance.GetDevices(), controller.Mapper,
                types.ToArray()), types.Any()).ShowDialog();
    ...
    private void CreateMappingControls()
    {
        foreach (var xInputType in XInputHelper.Instance.Buttons)
            Model.MapperButtonViews.Add(
                new MappingView(new MappingViewModel(new MappingModel(), controller, xInputType)));
        foreach (var xInputType in XInputHelper.Instance.DPad)
            Model.MapperDPadViews.Add(
                new MappingView(new MappingViewModel(new MappingModel(), controller, xInputType)));
        foreach (var xInputType in XInputHelper.Instance.Axes)
            Model.MapperAxisViews.Add(
                new MappingView(new MappingViewModel(new MappingModel(), controller, xInputType)));
    ...
    {
        foreach (var axisView in Model.XInputAxisViews) axisView.UpdateValues(controller.XInput);
        foreach (var buttonView in Model.XInputButtonViews) buttonView.UpdateValues(controller.XInput);
        foreach (var dPadView in Model.XInputDPadViews) dPadView.UpdateValues(controller.XInput);
    }
}
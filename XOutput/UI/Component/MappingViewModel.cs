...
public class MappingViewModel : ViewModelBase<MappingModel>
{
    ...
    public void Configure()
    {
        new AutoConfigureWindow(
            new AutoConfigureViewModel(new AutoConfigureModel(), InputDevices.Instance.GetDevices(), controller.Mapper,
                new[] { Model.XInputType }), false).ShowDialog();
    ...
    {
        if (Helper.DoubleEquals(mapperData.MinValue, Model.XInputType.GetDisableValue()) &&
            Helper.DoubleEquals(mapperData.MaxValue, Model.XInputType.GetDisableValue()))
        {
